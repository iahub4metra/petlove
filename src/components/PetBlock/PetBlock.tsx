...
export default function PetBlock() {
    ...
    const isAddPetPage = location === '/add-pet';
    ...
    return (
        <div
            className={`${
                isAddPetPage
                    ? 'flex justify-center xl:items-end bg-[center_37px] md:bg-position-[center_53px]'
                    : 'bg-bottom-right md:bg-position-[125px_bottom]'
            } relative rounded-[30px] md:rounded-[60px] shrink-0 xl:h-[654px] bg-[#F6B83D] recktangle bg-[url(/images/Rectangle.png)] bg-no-repeat bg-size-[300px_241px] md:bg-size-[528px_251px] xl:w-[592px] xl:bg-[url(/images/bg-pet-desc.png)] xl:bg-size-[512px_598px] xl:bg-position-[center_bottom]`}
        >
            <picture>
                <source
                ...
                <img
                    ...
                    alt={isRegister ? 'cat' : isLogin ? 'dog' : 'dog'}
                    className={`${
                        isAddPetPage
                            ? 'xl:h-[648px]'
                            : 'md:mr-[90px] md:ml-auto xl:mr-[20px] xl:h-[654px]'
                    }`}
                />
            </picture>
            {isAuthPage && (
